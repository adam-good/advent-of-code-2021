...
def _count_increases_imparative(measurements: list[int]) -> int:
    num_increases = 0
...
def _count_increases_functional(measurements: list[int]) -> int:
...
def _plot_compare_functions():
    ...
    for s in scales:
        ...
        t1 = time.time()
        sample_imp = sample_fn(_count_increases_imparative, s, 16)
        sample_fnc = sample_fn(_count_increases_functional, s, 16)
        t_diff = time.time() - t1
...
if __name__ == "__main__":
    ...
    print(_count_increases_imparative(measurements))
    print(_count_increases_functional(measurements))